...
describe('react-compare-app page', () => {
  ...
  it('should have the right title', () => {
    assert.strictEqual(homePage.getPageTitle(), 'React App');
  });
  ...
  it('should display table when all 4 items selected', () => {
    ...
    assert.equal($('//tr/th[2]').getText(), 'Cherry');
    assert.equal($('//tr/th[3]').getText(), 'Orange');
    assert.equal($('//tr/th[4]').getText(), 'Nuts');
    assert.equal($('//tr/th[5]').getText(), 'Strawberry');
    ...
    for (let i = 0; i < actualPrices.length; i++) {
      assert.equal(actualPrices[i].getText(), expectedPrices[i]);
    }
    ...
    for (let i = 0; i < expectedColors.length; i++) {
      assert.equal(actualColors[i].getAttribute('class'), expectedColors[i]);
    }
    ...
    for (let i = 0; i < expectedConditions.length; i++) {
      assert.equal(actualConditions[i].getText(), expectedConditions[i]);
    }
  });
